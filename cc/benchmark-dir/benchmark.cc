...
static constexpr uint64_t kInitCount = 3;
static constexpr uint64_t kTxnCount = 3;
static constexpr uint64_t kChunkSize = 1;
...
void SetThreadAffinity(size_t core) {
  ...
  size_t kCoreCount = std::thread::hardware_concurrency();
#ifdef _WIN32
...
void load_files(const std::string &load_filename,
  ...
  if (kInitCount != count) {
    printf("Init file load fail kInitCount=%ld and count=%ld\n", kInitCount,
           count);
    exit(1);
  }
...
int main(int argc, char *argv[]) {
  ...
  if (argc != kNumArgs + 1) {
    printf("Usage: ./benchmark <workload> <# threads> <load_filename> "
           "<run_filename>\n");
    exit(0);
  }
