...
TEST(CLASS, UpsertRead_Serial) {
  class Key {
  public:
    Key(uint64_t pt1, uint64_t pt2) : pt1_{pt1}, pt2_{pt2} {}
    ...
    inline bool operator==(const Key &other) const {
      return pt1_ == other.pt1_ && pt2_ == other.pt2_;
    }
    inline bool operator!=(const Key &other) const {
      return pt1_ != other.pt1_ || pt2_ != other.pt2_;
    }
  ...
  class Value {
  public:
    Value() : gen_{0}, value_{0}, length_{0} {}
  ...
  public:
    ...
    UpsertContext(const Key &key, uint8_t val) : key_{key}, val_{val} {}
  ...
  public:
    ...
    ReadContext(Key key, uint8_t expected) : key_{key}, expected_{expected} {}
...
TEST(CLASS, UpsertRead_Concurrent) {
  ...
  auto complete_callback = []() { complete = true; };

  result = store.ShiftBeginAddress(Address{kNewBeginAddress}, truncate_callback,
                                   complete_callback);
  ASSERT_TRUE(result);
...
TEST(CLASS, Rmw) {
  ...
  public:
    ...
    RmwContext(Key key, uint64_t incr) : key_{key}, incr_{incr}, val_{0} {}
...
TEST(CLASS, Rmw_Concurrent) {
  ...
  public:
    ...
    RmwContext(Key key, uint64_t incr) : key_{key}, incr_{incr} {}
    ...
    inline void RmwInitial(Value &value) { value.counter_ = incr_; }
    inline void RmwCopy(const Value &old_value, Value &value) {
      value.counter_ = old_value.counter_ + incr_;
    }
    inline bool RmwAtomic(Value &value) {
  ...
  auto read_worker1 = [](FasterKv<Key, Value, disk_t> *store_,
                         size_t thread_idx) {
    Guid session_id = store_->StartSession();
    for (size_t idx = 0; idx < kNumRecords / kNumThreads; ++idx) {
      auto callback = [](IAsyncContext *ctxt, Status result) {
  ...
  auto read_worker2 = [](FasterKv<Key, Value, disk_t> *store_,
                         size_t thread_idx) {
    Guid session_id = store_->StartSession();
    for (size_t idx = 0; idx < kNumRecords / kNumThreads; ++idx) {
      auto callback = [](IAsyncContext *ctxt, Status result) {
